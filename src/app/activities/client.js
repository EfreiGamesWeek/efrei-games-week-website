...
export default function ClientPage({ createActivity, deleteEnrollById, updateActivityById, enrollActivity, deleteActivityById }) {
	...
	useEffect(() => {
		async function fetchData() {
			...
			if (response != null) {
				...
				setOptions(options);
				const responseActivityEnrolled = await fetch("http://www.api.efreigamesweek.fr:8000/enroll/team/" + responseTeam.body._id).then((resp) => resp.json());
				const allActivityId = [];
		...
		async function fetchActivities() {
			const response = await fetch("http://www.api.efreigamesweek.fr:8000/activities/").then((resp) => resp.json());
			if (response == null) return;
			setActivities(response);
			const enrollByActivityID = {};
			const allEnroll = await fetch("http://www.api.efreigamesweek.fr:8000/enroll/").then((resp) => {
				return resp.json();
			});
			allEnroll.map((elem) => {
	...
	const enroll = (idActivity, numberOfContestantMax, numberOfContestantByTeam) => {
		enrollActivity(idActivity, myTeamInfo._id, myTeamInfo.name, JSON.stringify(selected), numberOfContestantMax, numberOfContestantByTeam);
		location.reload();
	};
				...
				<main className="w-7xl">
					<Dialog>
						<header className="grid grid-cols-3 grid-rows-1 gap-8 justify-between w-full ml-auto">
							<div></div>
							<h2 className="text-4xl font-bold mb-8 font-hungry">Les activités</h2>
							{userInfo == null ? null : userInfo.admin ? (
								<div className={"flex justify-end"}>
									<DialogTrigger asChild>
										<Button className={"cursor-pointer"}>Ajouter une activité</Button>
									</DialogTrigger>
								</div>
							) : null}
						</header>
							...
							<main>
								<div>
									<Field className="pb-4">
										...
										<Input id="name" required placeholder="Le nom de l'activité"></Input>
									</Field>
									<Field className="pb-4">
										<FieldLabel htmlFor="desc">Description de l'activité</FieldLabel>
										<Textarea id="desc" placeholder="La description de l'activité"></Textarea>
									</Field>
									<Field className="pb-4">
									...
									<Field className="pb-4">
										...
										<Input id="time" required placeholder="Les horaires de l'activité"></Input>
									</Field>
									<Field className="pb-4">
										<FieldLabel htmlFor="numberOfPointsAvailable">Points à gagner *</FieldLabel>
							...
							<DialogFooter>
								<section className="flex justify-between w-full">
									<div className="ml-auto flex gap-2">
										...
										<DialogClose asChild>
											<Button onClick={saveActivity} type="submit" className={"cursor-pointer"}>
												Sauvegarder
											</Button>
										</DialogClose>
									</div>
					...
					<section className="flex gap-4 w-full justify-center mb-8 flex-wrap">
						{activities == null && Object.keys(enrollByActivityID) != 0
							...
							: activities.map((elem, idx) => (
									<Dialog key={idx}>
										<Card key={idx} className="md:flex-1/4 md:basis-[30%] md:grow-0 justify-center">
											<CardHeader>
												...
												<h2>Nombre de personnes par équipe : {elem.numberOfContestantByTeam}</h2>
											</CardHeader>
											<CardContent>{elem.description}</CardContent>
											<CardFooter className="flex justify-between text-sm">
												<h2>
													Nombre d'inscrits : {enrollByActivityID[elem._id] == undefined ? 0 : enrollByActivityID[elem._id].length * elem.numberOfContestantByTeam}/{elem.numberOfContestantMax}
												</h2>
												{activitiesAlreadyEnrolled.includes(elem._id) ? (
													<Button
														onClick={() => {
															deleteEnrollById(enrollByActivityID[elem._id].find((o) => o.idTeam == myTeamInfo._id)._id);
															location.reload();
														}}
										...
										{userInfo == null ? null : userInfo.admin ? (
											<DialogContent className="sm:max-w-md">
												...
												<main>
													<div>
														<Field className="pb-4">
															<FieldLabel htmlFor="name">Nom de l'activité *</FieldLabel>
															<Input defaultValue={elem.name} id="name" required placeholder="Le nom de l'activité"></Input>
														</Field>
														<Field className="pb-4">
															<FieldLabel htmlFor="desc">Description de l'activité</FieldLabel>
															<Textarea defaultValue={elem.description} id="desc" placeholder="La description de l'activité"></Textarea>
														</Field>
														<Field className="pb-4">
															<FieldLabel htmlFor="organizer">Nom de l'organisateur *</FieldLabel>
														...
														<Field className="pb-4">
															<FieldLabel htmlFor="time">Horaires *</FieldLabel>
															<Input defaultValue={elem.time} id="time" required placeholder="Les horaires de l'activité"></Input>
														</Field>
														<Field className="pb-4">
															<FieldLabel htmlFor="numberOfPointsAvailable">Points à gagner *</FieldLabel>
												...
												<DialogFooter>
													<section className="flex justify-between w-full">
														<div className="ml-auto flex gap-2">
															...
															<DialogClose asChild>
																<Button onClick={() => updateActivity(elem._id, idx)} type="submit" className={"cursor-pointer"}>
																	Sauvegarder
																</Button>
															</DialogClose>
														</div>
											...
											<DialogContent className="sm:max-w-md">
												...
												<main>
													<Field className="pb-4">
														<FieldLabel htmlFor="contestants">Choisis tes {elem.numberOfContestantByTeam} champions *</FieldLabel>
														<MultiSelect
															value={selected}
															id="contestants"
															options={options}
												...
												<DialogFooter>
													<section className="flex justify-between w-full">
														<div className="ml-auto flex gap-2">
															...
															<DialogClose asChild>
																<Button disabled={selected.length != elem.numberOfContestantByTeam} onClick={() => enroll(elem._id, elem.numberOfContestantMax, elem.numberOfContestantByTeam)} type="submit" className={"cursor-pointer"}>
																	Sauvegarder
																</Button>
															</DialogClose>
														</div>
